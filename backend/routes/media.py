...
def _own_draft(draft_id, guest=False):
    member = None if guest else current_member()
...
def get_media(media_id):
    # Images/videos cannot send the custom guest header used by fetch requests.
    guest = is_guest_request() or request.args.get('guest') == '1'
    db = get_db()
    with db.cursor() as cursor:
        ...
        if row['post_id']:
            ...
            if not post or not can_read(member, post['board'], guest):
                abort(404)
        elif not _own_draft(row['draft_id'], guest):
            abort(404)
